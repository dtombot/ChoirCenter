...
exports.handler = async (event, context) => {
  ...
  if (missingVars.length > 0) {
    ...
    return {
      statusCode: 200, // Changed to 200 to avoid breaking frontend
      body: JSON.stringify({
        ga: { rows: [], error: `Missing environment variables: ${missingVars.join(', ')}` },
        gsc: { rows: [], error: `Missing environment variables: ${missingVars.join(', ')}` },
      }),
    };
  }
  ...
  try {
    ...
    return {
      statusCode: 200, // Graceful fallback
      body: JSON.stringify({
        ga: { rows: [], error: 'Authentication refresh failed: ' + refreshError.message },
        gsc: { rows: [], error: 'Authentication refresh failed: ' + refreshError.message },
      }),
    };
  }
